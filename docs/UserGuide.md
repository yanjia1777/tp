...
| `a/AMOUNT`  | Amount in dollars. | Positive number smaller than 1 million. It will be automatically   | | 
|             | Numbers after decimal points are in cents. | rounded to 2 decimal points if higher precision is given.
...
| `c/CATEGORY_NUMBER`| Category number | Please refer to the [available categories](#categoryList). If the   | `c/1` |
|             |                    | `CATEGORY_NUMBER` is not specified, the default `CATEGORY_NUMBER`    |  |
|             |                    | would be `c/7` which is `Others`.                                   |  |
| `i/INTERVAL`| How often one receives | String of either MONTH or YEAR. The string is not case-sensitive.   | `i/mOnTH`, `i/year`, `i/YEAR` |
|             | or pay for entries |                                                                     |  |
| `e/END_DATE` | End date of the recurring | Any of the [acceptable date formats](#dateFormat) from `2000-01-01` | `e/2016-02-13`, `e/2016-2-13`  |
|             |  period. One will not receive | to `2200-12-31` that is after the `d/DATE`. If the end date is not  |  |
|             | or pay for the entry from this date. | specified, the default date set would be forever (`2200-12-31`)     |  |
